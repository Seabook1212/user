...
func parseTagsAsZipkinOptions(t map[string]interface{}) []zipkin.SpanOption {
	...
	if val, ok := t[string(ext.SpanKind)]; ok {
		...
		// Preserve span.kind as an explicit tag for backends/UI that rely on tags.
		tags["span.kind"] = kind
	}
	...
	if val, ok := t[string(ext.PeerService)]; ok {
		...
		if serviceName != "" {
			// Preserve peer.service as an explicit tag in addition to remote endpoint metadata.
			tags["peer.service"] = serviceName
		}
	}
