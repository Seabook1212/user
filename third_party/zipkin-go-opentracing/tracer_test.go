...
func TestOTKindTagIsParsedSuccessfuly(t *testing.T) {
	...
	for _, tags := range tagCases {
		...
		if want, have := "server", spans[0].Tags["span.kind"]; want != have {
			t.Errorf("unexpected span.kind tag value, want %s, have %s", want, have)
		}
	}
}
...
func TestOptionsFromOTTags(t *testing.T) {
	...
	if want, have := "service_a", spans[0].Tags["peer.service"]; want != have {
		t.Errorf("unexpected peer.service tag value, want %s, have %s", want, have)
	}
